...
export async function deployNft(this: Deployer, nftCollection: NftCollectionType) {
  ...
  await this.ensurePreviousNftExists(nftCollection, toDeploy.id || 0)

  this.log(`[Deployer] NFT deploy started ${toDeploy.id} ${toDeploy.owner_address || ''}`)

  let toDeployId = toDeploy.id;

  while (await isNftExists(this.tonweb, nftCollection, toDeployId!)) {
    this.log(`[Deployer] NFT item already exists for token id ${toDeployId}.`);
    this.deployIndex++;
    toDeployId!++;
  }
  
  console.log('NFT deploying with id:', toDeployId);

  // Check if nft exists
  const nftItemAddress = await callTonApi<
    ReturnType<typeof nftCollection.getNftItemAddressByIndex>
  >(() => nftCollection.getNftItemAddressByIndex(toDeployId || 0))
  const nftItem = new NftItem(this.tonweb.provider, {
    address: nftItemAddress,
  })
  console.log(nftItemAddress.toString(true, true, true));
  ...
  const walletAddress = await this.wallet?.getAddress() || new TonWeb.utils.Address('');
  ...
  if (!itemInfo) {
    throw new Error(`[Deployer] no nft item info ${toDeployId}`)
  }
  ...
  this.deployIndex++
  this.log(`[Deployer] NFT deployed ${toDeployId}`)
}
