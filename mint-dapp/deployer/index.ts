...
class Deployer {
  ...
  private checkProcess: (value: boolean) => void;
  private checkSuccess: (value: boolean) => void;
  protected log: (log: string) => void
  protected workInterval?: number | NodeJS.Timer

  async initializeTonWeb() {
    const endpoint = await getHttpEndpoint({
      network: "testnet",
    });
    const tonApiEndpoint = endpoint;

    this.tonweb = new TonWeb(new TonWeb.HttpProvider(tonApiEndpoint));
  }

  constructor(config: Config, nfts: Nft[], log?: (log: string) => void, checkProcess?: (value: boolean) => void, checkSuccess?: (value: boolean) => void) {
    this.config = config
    this.nfts = nfts
    ...
    this.initializeTonWeb().catch(err => console.error("Failed to initialize TonWeb:", err));
    const tonApiEndpoint = 'https://ton.access.orbs.network/44A2c0ff5Bd3F8B62C092Ab4D238bEE463E644A2/1/testnet/toncenter-api-v2/jsonRPC';
  ...
  stop() {
    if (typeof this.workInterval === 'number') {
      ...
      clearTimeout(this.workInterval as any)
    }
  }
  ...
  async work() {
    ...
    if (this.nfts.length <= this.deployIndex) {
      this.log(`[Deployer] Got no more nfts to deploy`)
    ...
    if (this.collection) {
      ...
      this.log(`[Deployer] collection not found`)
      this.checkProcess(false);
    }
  }
